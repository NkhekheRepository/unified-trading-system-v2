...
### Option 1: Testnet Mode (Recommended for First Run)

This runs the system connected to Binance Futures Testnet.

```bash
# Make sure you have your Testnet API keys in .env
cp .env.example .env
...
This runs the system connected to Binance Futures Live (use with caution).

```bash
# Make sure you have your Live API keys in .env
cp .env.example .env
# Edit .env with your Live keys from https://www.binance.com/en/future
python3 run_enhanced_live.py
```

### Option 3: Paper Trading (Simulated)

This runs the system in simulated mode with no real money at risk.
The system defaults to PAPER mode if no API keys are provided.
...
```bash
# For testnet/live mode, check the respective logs
tail -f logs/testnet/output.log   # for testnet
# tail -f logs/live/output.log    # for live
# For paper trading, check:
# tail -f logs/binance_trading.log
```

You should see entries indicating the system is initializing and beginning trading cycles.

### Check Health Endpoints

```bash
# Check system health (default port 8080)
...
| 2-10 min | Market data begins, belief state estimation, initial signal generation |
| 10-20 min | First trading signals generated, risk assessment, potential order execution |
